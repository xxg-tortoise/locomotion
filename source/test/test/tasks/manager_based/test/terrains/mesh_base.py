...
def tianzi_grid_terrain(difficulty: float, cfg: "MeshTianziTerrainCfg") -> tuple[list[trimesh.Trimesh], np.ndarray]:
    ...
    vertical_centers_x, free_cell_centers_x = _compute_axis_layout(
        cfg.size[0], num_vertical_beams, beam_width, x_margin
    )
    horizontal_centers_y, free_cell_centers_y = _compute_axis_layout(
        cfg.size[1], num_horizontal_beams, beam_width, y_margin
    )
    ...
    beam_span_x = cfg.size[0]
    beam_span_y = cfg.size[1]

    if cfg.add_outer_frame and frame_width > 0.0:
        inner_size = (cfg.size[0] - 2.0 * frame_width, cfg.size[1] - 2.0 * frame_width)
        if inner_size[0] <= 0.0 or inner_size[1] <= 0.0:
            raise ValueError(
                f"Frame width {frame_width} is too large for terrain size {cfg.size}."
            )
        frame_center = (0.5 * cfg.size[0], 0.5 * cfg.size[1], 0.5 * beam_height)
        meshes_list += make_border(cfg.size, inner_size, beam_height, frame_center)
    ...
    for center_x in vertical_centers_x:
        beam_pos = (center_x, 0.5 * cfg.size[1], 0.5 * beam_height)
    ...
    for center_y in horizontal_centers_y:
        beam_pos = (0.5 * cfg.size[0], center_y, 0.5 * beam_height)
        beam = trimesh.creation.box(horizontal_dims, trimesh.transformations.translation_matrix(beam_pos))
        meshes_list.append(beam)

    if cfg.origin_on_free_cell:
        origin_x = free_cell_centers_x[np.argmin(np.abs(free_cell_centers_x - 0.5 * cfg.size[0]))]
        origin_y = free_cell_centers_y[np.argmin(np.abs(free_cell_centers_y - 0.5 * cfg.size[1]))]
    else:
